...
public class ExcelController {
	...
	public Optional<List<Person>> readRowsExcel() {
	...
	public void writeOutputFile(List<Person> persons) {
		try (FileInputStream fileInputStream = new FileInputStream(processingPath);
			...
			String outputFilePath = PageEnum.PATH_EXCEL_OUTPUT.getValue() + PageEnum.NAME_EXCEL_FILE.getValue() + "_"
					+ new SimpleDateFormat("yyyy-MM-dd_HH-mm").format(new Date()) + ".xlsx";
	...
	private String getConvertedCellData(Cell cell) {
		StringBuilder cellReturn = new StringBuilder();

		try {
			if (cell != null) {
				if (cell.getCellType().equals(CellType.NUMERIC)) {
					cellReturn.append(String.valueOf((long) cell.getNumericCellValue()));
				} else if (cell.getCellType().equals(CellType.STRING)) {
					cellReturn.append(cell.getStringCellValue());
				}
			}
		...
		return cellReturn.toString().trim();
	}
